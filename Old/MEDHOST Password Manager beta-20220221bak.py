...
def FindCyberArkAccount(Token,Username=None,SearchType='contains',SafeName='',ArkURL = "https://cyberark.medhost.com/PasswordVault/API"):
    ...
    if not Username or Username == 'Search for accounts':
        ...
        URL    = ArkURL + "/Accounts" + filter
        Response = (get(url=URL,headers=Header)).json()
        return Response
    else:
        filter = f"&filter=safeName+eq+{SafeName.split(' '*50)[1]}" if SafeName and SafeName != 'Safe Name' else ''
        
        URL  = ArkURL + "/Accounts" + '?search=' + Username + filter + '&searchtype=' + SearchType
        Response = (get(url=URL,headers=Header)).json()
...
def GetCyberArkPassword(AccountId,Token,UserName,Reason="MEDHOST Password Manager Update",ArkURL="https://cyberark.medhost.com/PasswordVault/API"):
...
def SaveToConfig(Inputs,Slots,WorkingDirectory,pin,ArkInfo,LPInfo,window,Print=True):
        ...
        config['CyberArk']['Accounts']                    = str(window['CATable'].Values)
...
if ArkToken:
    try:
        ...
        ArkSafes   = GetCyberArkSafes(ArkToken)
    except:
        ArkAccount = None
        ArkSafes   = None
else:
...
if not ArkAccount or 'ErrorMessage' in ArkAccount or not ArkSafes:
    CATable = ConfigCATable if ConfigCATable else [['Login to see CyberArk Accounts here']]
    CASafes = ['Safe Name']
else:
    CATable  = ConfigCATable if ConfigCATable else [[_['userName'],_['safeName'],_['id']] for _ in ArkAccount['value']]
    CASafes = ['Safe Name'] + [_['safeName']+' '*50+_['safeUrlId'] for _ in ArkSafes['value']]
...
LastPassOptions = [
    [sg.Text("⩠ Login",justification="Left",size=(5, 1),font=("Helvetica",10+Fm),**ee,visible=True,k='LPLoginToggle'),sg.Text("LastPass",justification="center",size=(35+Sm*2, 1),font=(f"Helvetica {12+Fm} bold"))],
    [sg.pin(sg.Column([
    ...
    [sg.Button(button_text='Choose For Me',key="LPAutoChoose",visible=False,p=(5,3)),sg.Push(),sg.Button(button_text='Clear All',key="LPClear",p=(5,3))]
]
#endregion
...
Tp  = 3 if sg.running_mac() else 10
GCOptions = [
...
#region CyberArk
Pd = 11 if sg.running_mac() else 5
...
CAOptions = [
    ...
    [sg.pin(sg.Column([
        ...
        [sg.Text('Username:', size=(8, 1),p=((5,0),3),tooltip=UserNameToolTip),sg.Input(default_text=ConfigUsername,k="ArkUser",tooltip=UserNameToolTip,p=((5,7),3),s=(16+Sm,1)),
        sg.Text('Password:', size=(8, 1),p=((5,0),3),tooltip=PasswordToolTip),sg.Input(password_char="*",k="ArkPass",tooltip=PasswordToolTip,default_text=AppData['ArkPass'],s=(16+Sm,1)),
        sg.Push(),sg.Button(button_text='Login',key="CALogin")]
    ],p=(0,(0,Pcl)),k="ArkLoginInfo",visible=False))],
    [sg.Text('Select an Account to check-out and sync:', size=(32,1),p=((5,0),0)),sg.Push()], #,sg.Button('Refresh',k="CARefresh",font=('Helvetica',10))],
    [sg.Table(CATable,['Username','Safe'],auto_size_columns=False,justification='left'  ,visible_column_map=[True,True,False] ,num_rows=9, k='CATable',col_widths=[25+Tcs, 24+Tcs],p=(5,(3,0)),**InputColor,tooltip=BGUserToolTip,select_mode=sg.TABLE_SELECT_MODE_BROWSE)],
    [sg.Combo(['Contains','StartsWith'],'Contains',s=(9, 1),k='CAsType'),sg.Input('Search for accounts',key="ArkQuery",s=(25,1)),sg.Combo(CASafes,'Safe Name',s=(15, 1),k='CASafes'),sg.Button('Search',k="CASearch",bind_return_key=True,target='ArkQuery')],
    [sg.Text('Reason for check-out', size=(16+Sm, 1),p=(5,(3,Pd)),tooltip=BGUserToolTip),sg.Input(CAeReason,key="CAReason",s=(45,2))]
]
#endregion
...
OkSlotSelection = [
    ...
    [sg.Column(SlotSelector,k="SlotSelector",p=0,visible=not KWTrue)]
]
#endregion
...
LVAcctDetails = [
    ...
    [sg.Multiline(s=(63,7),p=((3,3),(1,0)),k="LVNote",write_only=True,disabled=True,**AcctColor,**AcctFont)]
]
#endregion

#region Local vault

Vtp = 19 if sg.running_mac() else 13
...
VaultLayout = [
    ...
    [sg.Table(LVTable,['Name','Username','Url','Id','Group','Notes','Password',],auto_size_columns=False,justification='left'  ,visible_column_map=[True,True,False,False,False,False,False] 
     ,num_rows=18, k='LVTable',col_widths=[24+Tcs, 25+Tcs],enable_events=True,select_mode=sg.TABLE_SELECT_MODE_BROWSE,**InputColor)],
    [sg.Combo(['Type','Password','SecureNote'],'Type',s=(10, 1),p=(5,(3,Vtp)),**ee,k='LVsType'),sg.Input('Search for accounts',key="LVQuery",p=(5,(3,Vtp)),s=(32,1),**ee),
     sg.Combo(LVGroups,'Group/Folder',s=(15, 1),p=(5,(3,Vtp)),k='LVsGroup',**ee)],
    [sg.TabGroup([
...
layout = [
    ...
    [sg.Text('Sync From:', size=(8+Sm, 1)),sg.Radio('CyberArk','PFrom',k='CAFrom',enable_events=True,default=CATrue),sg.Radio('LastPass','PFrom',k='LPFrom',enable_events=True,default=LPFrom)
    ,sg.Radio('Chrome','PFrom',k='GCFrom',enable_events=True,default=False),sg.Radio('Edge','PFrom',k='MEFrom',enable_events=True,default=False),sg.Push()],
    ...
    [sg.Button(button_text='Save',key="Save",tooltip=SaveToolTip),sg.Push(),sg.pin(sg.Button(button_text='Check In',key="CheckIn"))
     ,sg.Submit(submit,tooltip=SubmitToolTip,k='Submit')],
    [sg.HSep()],
    [sg.TabGroup([
        [sg.Tab('Output',OutPutSection,k='OutPutSection'),sg.Tab('LastPass',LastPassOptions,k='LPOptions',visible=LPTrue and not LPFrom), 
...
window = sg.Window(Title, MainTabs,font=("Helvetica", 10+Fm),finalize=True)
#endregion
...
if ConfigCALogin:
    window['CATable'].NumRows = 5
    if sg.running_mac(): window['CATable'].update(num_rows=5)
    window['ArkLoginInfo'].update(visible=not window['ArkLoginInfo']._visible)
    window['CALoginToggle'].update(value='⩢ Login')
    if not sg.running_mac(): window['CATable'].update(num_rows=5)
...
while True:
    try:
        ...
        if event:
            window['-OUTPUT-2'].update(value=window['-OUTPUT-'].get())
        ...
        if event in ['CALoginToggle','CALogin',"CALoginSearch"]:
            if window['ArkLoginInfo']._visible:
                window['CATable'].NumRows = 9
                if sg.running_mac(): window['CATable'].update(num_rows=9)
                window['CALoginToggle'].update(value='⩠ Login')
                window['ArkLoginInfo'].update(visible=not window['ArkLoginInfo']._visible)
                if not sg.running_mac(): window['CATable'].update(num_rows=9)
        ...
        if event in ['LVLogin','LVRefresh']:
            ...
            window['LVTable'].NumRows = 18
            if sg.running_mac(): window['LVTable'].update(num_rows=18)
            window['LVLoginToggle'].update(value='⩠ Login')
            window['LVLoginInfo'].update(visible=not window['LVLoginInfo']._visible)
            if not sg.running_mac(): window['LVTable'].update(num_rows=18)
        ...
        if event == 'LVLoginToggle':
            if window['LVLoginInfo']._visible:
                ...
                window['LVTable'].NumRows = 11
                if sg.running_mac(): window['LVTable'].update(num_rows=11)
                window['LVLoginInfo'].update(visible=not window['LVLoginInfo']._visible)
                window['LVLoginToggle'].update(value='⩢ Login')
                if not sg.running_mac(): window['LVTable'].update(num_rows=11)
        ...
        if event == 'CARefresh':
            ...
            if not ArkAccount or 'ErrorMessage' in ArkAccount:
                ...
                ArkSafes   = GetCyberArkSafes(ArkToken)
                CASafes = ['Safe Name'] + [_['safeName']+' '*50+_['safeUrlId'] for _ in ArkSafes['value']]
                window['CASafes'].update(values=CASafes)
                CATable = [[_['userName'],_['safeName'],_['id']] for _ in ArkAccount['value']]
                window['CATable'].update(values=CATable)
                
        if event in ['CALogin','CALoginReturn','CALoginCheckIn',"CALoginSearch"]:
            if event == 'CALogin':
                window['-OUTPUT-'].update('')
                window['OutPutSection'].select()
                window.perform_long_operation(lambda:GetCyberArkLogin(ArkUser=Inputs["ArkUser"],ArkPass=Inputs["ArkPass"]), "CALoginReturn")
                ...
                ArkAccts = FindCyberArkAccount(ArkToken,Inputs['ArkQuery'],Inputs['CAsType'],Inputs['CASafes'])
                ...
                ArkSafes   = GetCyberArkSafes(ArkToken)
                CASafes = ['Safe Name'] + [_['safeName']+' '*50+_['safeUrlId'] for _ in ArkSafes['value']]
                CASafe = Inputs['CASafes'] if Inputs['CASafes'] in CASafes else 'Safe Name'
                window['CASafes'].update(values=CASafes)
                window['CASafes'].update(value=CASafe)                
                CATable = [[_['userName'],_['safeName'],_['id']] for _ in ArkAccts['value']]
        ...
        if event in ['CASearch','CALoginSearch']:
        ...
        if event in ['CheckIn','CALoginCheckIn']:
            ...
            CheckInResponse = CheckInCyberArkPassword(AccountId=BGUser[2], Token=ArkToken, UserName=BGUser[0])
...
#region Password Sync
        if event == "Submit" or FunctionReturn:
            window['OutPutSection'].select()
            ...
            if Inputs['CAFrom']:
                if not FunctionReturn and not AppData['ArkToken']: 
                    ...
                    if ArkAccount and 'ErrorMessage' in ArkAccount and CALoginAttempt == 0 and Inputs["ArkUser"] and Inputs["ArkPass"]:
                        ...
                        print("Unable Find CyberArk Account for " + BGUser[0])
                    ...
                    ArkAccountPass = GetCyberArkPassword(AccountId=BGUser[2], Token=ArkToken, UserName=BGUser[0],Reason=Inputs['CAReason'])
                    if ArkAccountPass and isinstance(ArkAccountPass,dict) and 'ErrorMessage' in ArkAccountPass:
                        print(ArkAccountPass['ErrorMessage'])
                        print(f'Unable to retreive password from CyberArk for {BGUser[0]}')
                        continue
                    ...
                    if Inputs['LP']:
                        ...
                        elif BGUser[0] in [_.name for _ in LastPass.accounts]:
                            Account = [_ for _ in LastPass.accounts if _.name == BGUser[0]][0]
                            LastPass.UpdateAccount(Account.id,password=ArkAccountPass,group='MEDHOST Password Manager\\CyberArk',Account=Account)
                        else:    
                            notes="Created By MEDHOST Password Manager"
                            LastPass.NewAccount(BGUser[0],BGUser[0],ArkAccountPass,'MEDHOST Password Manager\\CyberArk',notes=notes)
                        if LastPass.accounts:
                            LVTable = [[_.name,_.username,_.url,_.id,_.group,_.notes,_.password] for _ in LastPass.accounts]
                            window['LVTable'].update(values=LVTable)
                            LPList = [(_.name + (" " * 120) + str(_.id)) for _ in LastPass.accounts]
