...
def SwapContainers(window,C1,C2):
    ...
    newwindow = sg.Window(Title, window.Rows,location=location,finalize=True,font=window.Font,keep_on_top=True)
    window.close()
    newwindow.fill(values)
    for box in listboxes: newwindow[box['Key']].update(values=box['Values'])
...
OutPutSection = [
    [sg.Output(size=(62+Im,15), key='-OUTPUT-',visible=True,expand_x=True,echo_stdout_stderr=True,pad=((5,5),(5,3)),tooltip=OutputToolTip)],
