...
def UpdateOnlyKey(ArkUser,ArkPass,BGUsername,OK_Keyword):
    #for param in (ArkUser,ArkPass,BGUsername,OK_Keyword):
    #    if not bool(param):
     #       print("All fields must be compeleted, fill in all empty fields")
     #       return

    try:
        OnlyKey(connect=True).read_bytes(timeout_ms=1000)
...
while True: 
    ...
    if event == "Submit":
        window['-OUTPUT-'].update('')
        window['SmallOut'].update('')
        ArkUser=Inputs["ArkUser"];ArkPass=Inputs["ArkPass"];BGUser=Inputs["BGUser"];OK_Keyword=Inputs["OK_Keyword"]
        window.perform_long_operation(lambda:UpdateOnlyKey(ArkUser=ArkUser,ArkPass=ArkPass,BGUsername=BGUser,OK_Keyword=OK_Keyword), "FunctionReturn")
        continue
window.close()
#endregion